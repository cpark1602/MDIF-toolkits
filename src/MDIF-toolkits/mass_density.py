...
class Mass_density:
    def __init__(self, universe, box, print_results_path, pbc, bin_size, dim, start=None, stop=None, step=None): 
    ...
    def _get_number_density(self, t, atom_name, dim, bin_size, boxL, start_end_skip):
        ...
        for ts in t.trajectory[0:1]:
            ...
            frames += 1 
        # run over frames
        for ts in t.trajectory[start_end_skip[0]:start_end_skip[1]:start_end_skip[2]]:
            locpres= t.coord.positions
            atom_pos = locpres[atom_index-1, dim_ndx]
            hist_atom, hist_edges = np.histogram(atom_pos, bins=n_bins, range=(0, L))
            hist_new_atom += hist_atom
    ...
    def _get_densityProfile(self, atom_name):
        ...
        hist_edges, number_density = self._get_number_density(self.u, atom_name, self.dim, self.bin_size, self.box, self.start_end_skip)
        return hist_edges, number_density
