...
class ACF:
    ...
    def _single_run(self, start, stop, step):
        ...
        hb_acf_results_IF = np.zeros_like(
            np.arange(start, stop, step), dtype=np.float32
        )
        hb_acf_results_BULK = np.zeros_like(
            np.arange(start, stop, step), dtype=np.float32
        )
        ...
        for ts in self.u.trajectory[start:stop:step]:
            ...
            time_fs = (
                ts.frame * stepsize
            )  # * self.step; ts.frame already include the step
            ...
            for h_tag in self.Hatoms:
                oxygens_for_Htagging = self.ns_acceptors.search(
                    h_tag, 5.0
                )  # self.box[0]*2 self.box[0], search oxygens within the simulation box
                ...
                if closest_o_near_h_tagged_index in self.dict_oxygen_with_h_tagged:
                    ...
                    self.dict_oxygen_with_h_tagged[closest_o_near_h_tagged_index] = [
                        h_tag
                    ]
            ...
            for key, value in self.dict_oxygen_with_h_tagged.items():
                d = self._s1[
                    self.dict_Oatom_index_to_i[key]
                ]  # use a new dictionary which is sorted according to the self._s1 list.
                        ...
                        if d.index == a.index:
                            ...
                            if (d.index, h.index, a.index) in already_found:
                                ...
                                angle_rad = distances.calc_angles(
                                    h.position, d.position, a.position, box=self.box
                                )

                                angle_OHO_rad = distances.calc_angles(
                                    d.position, h.position, a.position, box=self.box
                                )
                                ...
                                dist_O1_H = distances.calc_bonds(
                                    d.position, h.position, box=self.box
                                )
                                ...
                                if self.HBs_criteria == "Luzar":
                                    if (
                                        angle <= self.angle
                                        and dist <= self.cutoff_dist_donor_acceptor
                                    ):
                                        already_found[(d.index, h.index, a.index)] = (
                                            True
                                        )
                                        ...
                                        if len(value) == 3:
                                            dist_1 = distances.calc_bonds(
                                                h.position, d.position, box=self.box
                                            )
                                            dist_2 = distances.calc_bonds(
                                                h.position, a.position, box=self.box
                                            )
                                        ...
                                        if (
                                            ...
                                            and d.position[0] <= self.cutoff_BULK[1]
                                        ):
                                            if frame == start:
    ...
    def run(self, **kwargs):
        ...
        # Prepare
        hb_acf_results_IF_global = np.zeros_like(
            np.arange(window_list[0], window_list[1], self.step), dtype=np.float32
        )
        hb_acf_results_IF_acf_global = np.zeros_like(
            np.arange(window_list[0], window_list[1], self.step), dtype=np.float32
        )

        hb_acf_results_BULK_global = np.zeros_like(
            np.arange(window_list[0], window_list[1], self.step), dtype=np.float32
        )
        hb_acf_results_BULK_acf_global = np.zeros_like(
            np.arange(window_list[0], window_list[1], self.step), dtype=np.float32
        )

        c_dot_if_global = np.zeros_like(
            np.arange(window_list[0], window_list[1], self.step), dtype=np.float32
        )
        c_dot_if_2_global = np.zeros_like(
            np.arange(window_list[0], window_list[1], self.step), dtype=np.float32
        )
